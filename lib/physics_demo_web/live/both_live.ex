defmodule PhysicsDemoWeb.BothLive do
  ...
  def handle_info("move-ball", socket) do
    ...
    cond do
      past_top?(moved_ball) or past_bottom?(ball_center) -> bounce_ball(socket, :vertical)
      past_right?(moved_ball) -> bounce_ball(socket, :horizontal)
      hit_paddle?(moved_ball, bat_anchor) -> bounce_ball(socket, :horizontal)
      past_left?(moved_ball) -> reset_ball(socket)
  ...
  defp bounce_ball(socket, direction) do
    ...
    bounced_velocity = Physics.bounce(ball_velocity, direction)
